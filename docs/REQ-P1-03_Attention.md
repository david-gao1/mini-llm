...
Transformer 的核心能力来自自注意力机制——让序列中的每个 token 能"看到"其他 token 并动态计算相关性权重。但 GPT 是**自回归**模型，生成第 t 个 token 时只能依赖 0..t-1 的信息，不能偷看未来。因此需要：

- **因果掩码（causal mask）**：上三角矩阵将未来位置的注意力权重置零
- **多头并行**：将隐藏维度拆成多个头，每个头学习不同的注意力模式（如语法关系、语义关联等）

如果不加因果 mask，模型训练时会利用 target 信息，导致推理时无法自回归生成。如果只用单头注意力，模型的表达能力受限。

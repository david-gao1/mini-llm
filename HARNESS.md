...
| P3-02 | 训练监控与对照工具：`finetune_instruction.py` 的 `eval_val_batches` / `epoch_val_full` / `val_loss_full`；[`compare_instruction_generate.py`](compare_instruction_generate.py)；[`eval_instruction_loss.py`](eval_instruction_loss.py)；配置 [`configs/config_instruction_train_small.json`](configs/config_instruction_train_small.json) | 见 [`docs/REQ-P3-02_InstructionSFTEvalAndQuality.md`](docs/REQ-P3-02_InstructionSFTEvalAndQuality.md) | L0：同上 `pytest`；L2：`uv run python finetune_instruction.py --config configs/config_instruction_small.json --eval-val-only --eval-checkpoint runs/instruction_sft_small/checkpoint_best.pt`；L1：`uv run python compare_instruction_generate.py --pretrained runs/gpt2_small_wikitext103/checkpoint_best.pt --sft runs/instruction_sft_small/checkpoint_best.pt --prompt-file docs/prompts/instruction_compare_sample.txt`；L3：正式训 `uv run python finetune_instruction.py --config configs/config_instruction_train_small.json` | 无 NaN；日志含 `val_loss_full`；对照脚本退出码 0；本轮以 Small checkpoint 为底座 | done |

**Backlog**：**DPO / 偏好微调** → [`docs/REQ-P3-01_Ch07InstructionSFT.md`](docs/REQ-P3-01_Ch07InstructionSFT.md) §9（不阻塞 P3-01 SFT）。P3-02 已以 Small 底座收口；自动评分 / 批量导出 JSON 仍为非阻塞 backlog。

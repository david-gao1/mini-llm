...
词典（tokenizer）、传送带（loader）、会扭头看的模块（attention）都有了。**GPT 模型**就是把它们装进一辆车：先把编号变成向量并贴上「第几个位置」的标签，再叠许多层「关注度 + 小型前置神经网络」，最后在车尾巴挂一块 **和整本词典一样宽的成绩条**（LM head），告诉你下一个词谁的分最高。

### 最关键的一句话

> **输入**一句 token 编号矩阵 `[B, T]`，**输出**每个位置上「下一个词」在整张词表上的得分 `[B, T, vocab_size]`——后面训练和生成都围着这个形状转。

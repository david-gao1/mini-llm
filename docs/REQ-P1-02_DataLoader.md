...
语言模型的训练目标是"给定前 T 个 token，预测第 T+1 个"。原始语料是一整段文本（可能数万字），需要解决三件事：

1. **切分**：将长文本切成固定长度的 (input, target) 对，其中 target 是 input 右移一位——这就是"next token prediction"的监督信号
2. **加载**：语料文件可能在本地、在同级书本仓库、或需要从网络下载，需要一个统一的加载入口
3. **分割**：将语料分为训练集和验证集，评估模型是否过拟合

如果不做滑动窗口切分，就无法构造自回归训练所需的 (input, target) 对；如果不做 train/val 分割，就无法在训练过程中监控泛化能力。

...
分词器、数据加载、注意力、模型四大模块分别完成了各自的职责，但此刻它们还是独立的零件。需要一个**训练入口**把它们串成完整的预训练流水线：

1. 读取配置 → 设置随机种子
2. 加载语料 → 构建 train/val DataLoader
3. 初始化 GPTModel 与 AdamW 优化器
4. 训练循环：前向 → 计算 loss → 反向 → 参数更新
5. 定期评估 train/val loss、保存 checkpoint、打印生成样本

如果不完成这一步，前面所有模块都只是"能跑单测但没有真正训练过"的组件。**这是打通 M1 闸门的关键——验证从数据到 loss 到 checkpoint 的完整链路。**

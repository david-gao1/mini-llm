...
**基础训练已经能跑通**，但就像小车只在小区里兜圈：**train 超低、val 却在变差**——典型「背课文」过拟合；Mac 上 CUDA 不可用还会 **白跑 CPU**；采样若只会 **永远选第一名**，生成会像坏掉的复读机。

### 最关键的一句话

> **不改 m01–m05**，只在 `train.py` / `config` 里加：**MPS、梯度裁剪、学习率调度、early stopping、更像话的采样**，让验证集更可信、机器别空转。

### 当时仪表盘上亮了什么灯（简述）

| 指标 | 现象 |
|------|------|
| train_loss | 10.8 → **0.005**（模型把 ~20KB 语料几乎背下来了） |
| val_loss | 6.1 → **7.2**（Epoch 30 后往上走，泛化变差） |
| 生成文本 | Epoch 30 后固定复述同一段，缺少变化 |

此外还有：**M3 Max 没用上 MPS**；**贪心采样**看不出模型的多样性。

这些问题不影响「能跑通」，但影响「跑得好」。治理过程本身是学习内容。

...
作文写了一半停在半路：**生成**就是模型一个字一个字往下补。**贪心**等于永远选「下一个词排行榜第一名」，省事但容易车轱辘话；**温度**像往抽签筒里加抖动；**top-k** 先把太差劲的选项划掉再抽。

### 最关键的一句话

> 给定 **prompt + checkpoint**，**自回归续写**；提供 **`generate_text_simple`**（贪心）给训练时瞄一眼，**`generate`（temperature + top-k）** 给更像人话的文本。**闸门 M2** 需要证明「加载权重 → 能吐出非空可读片段」。
